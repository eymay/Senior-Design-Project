...
Add the specifications of the instruction to the RISCVInstrInfoCrypt.td file. Here we created a new class of instruction named as ALU\_rrr. MLA instruction requires three source registers and is defined to be ALU type so the specifications are:
...
\end{lstlisting}
\subsection{Newly Introduced ALU\_rrr TableGen Class}
...
\begin{itemize}
    \item If the instruction has no side effect, hasSideEffects will be 0.
    \item If there is no need or possibility to load data from memory, mayLoad will be 0.
    \item If there is no need or possibility to store data from memory, mayStore will be 0.
\end{itemize}
...
%TODO make sure MIR is explained previously
